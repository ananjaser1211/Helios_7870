...
int sec_ts_firmware_update_on_probe(struct sec_ts_data *ts, bool force_update)
{
	...
	if (gpio_is_valid(ts->plat_data->tsp_connect)) {
		if (gpio_get_value(ts->plat_data->tsp_connect)) {
			input_info(true, &ts->client->dev, "%s: skip %scalibration (not connected)\n",
						__func__, skip_firmup == 0 ? "skip fw update & " : "");
			skip_firmup = 1;
			restore_cal = 0;
		}
	}
