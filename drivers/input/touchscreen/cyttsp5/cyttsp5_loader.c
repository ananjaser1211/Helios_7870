		...
		int fw_size)
{
	...
	dev_info(dev, "%s: cmd->request_exclusive = %p\n",
		...
		struct cyttsp5_touch_firmware *fw)
{
	...
	if (fw_file_ver->hw_ver >= 0x03 && sti->hw_version < 0x03) {
		dev_info(dev, "%s: old panel, skip upgrade\n", __func__);
		return 0;
	}
...
static int upgrade_firmware_from_builtin(struct device *dev)
{
	...
	dev_info(dev, "%s: Enabling firmware class loader built-in\n",
		__func__);
	...
	dev_info(dev,"%s : finish fw update\n",__func__);
...
#endif
	dev_info(dev, "%s: FW upgrade start\n", __func__);
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_FW_UPGRADE
