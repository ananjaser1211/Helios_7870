...
#ifdef CONFIG_SENSORS_ABOV_A96T316
...
#ifdef CONFIG_SENSORS_ABOV_A96T316	//temp
				      ...
				      const char *buf, size_t count)
{
	...
	sscanf(buf, "%2d", &keyboard_mode_on);
	input_info(true, &client->dev, "%s : %d\n",
...
#ifdef CONFIG_TOUCHKEY_GRIP
static DEVICE_ATTR(grip_threshold, S_IRUGO, grip_threshold_show, NULL);
static DEVICE_ATTR(grip_total_cap, S_IRUGO, grip_total_cap_show, NULL);
static DEVICE_ATTR(grip_sar_enable, S_IRUGO | S_IWUSR | S_IWGRP, NULL, grip_sar_enable);
static DEVICE_ATTR(grip_sw_reset, S_IRUGO | S_IWUSR | S_IWGRP, NULL, grip_sw_reset);
static DEVICE_ATTR(grip_earjack, S_IRUGO | S_IWUSR | S_IWGRP, NULL, grip_sensing_change);
static DEVICE_ATTR(grip, S_IRUGO, grip_show, NULL);
static DEVICE_ATTR(grip_baseline, S_IRUGO, grip_baseline_show, NULL);
static DEVICE_ATTR(grip_raw, S_IRUGO, grip_raw_show, NULL);
static DEVICE_ATTR(grip_gain, S_IRUGO, grip_gain_show, NULL);
static DEVICE_ATTR(grip_check, S_IRUGO, grip_check_show, NULL);
#ifndef CONFIG_SAMSUNG_PRODUCT_SHIP
static DEVICE_ATTR(grip_sar_only_mode,  S_IRUGO | S_IWUSR | S_IWGRP,
						NULL, grip_mode_change);
static DEVICE_ATTR(grip_sar_press_threshold,  S_IRUGO | S_IWUSR | S_IWGRP,
						NULL, grip_sar_press_threshold_store);
static DEVICE_ATTR(grip_sar_release_threshold,  S_IRUGO | S_IWUSR | S_IWGRP,
...
static DEVICE_ATTR(grip_chip_name, S_IRUGO, grip_chip_name, NULL);
static DEVICE_ATTR(grip_firm_version_phone, S_IRUGO, bin_fw_ver, NULL);
static DEVICE_ATTR(grip_firm_version_panel, S_IRUGO, read_fw_ver, NULL);
static DEVICE_ATTR(grip_firm_update, S_IRUGO | S_IWUSR | S_IWGRP, NULL,
			grip_fw_update);
static DEVICE_ATTR(grip_firm_update_status, S_IRUGO | S_IWUSR | S_IWGRP,
			grip_fw_update_status, NULL);
...
static struct device_attribute *sec_touchkey_attributes[] = {
	...
	&dev_attr_touchkey_recent_raw,
	&dev_attr_touchkey_back_raw,
	&dev_attr_grip_chip_name,
	...
	&dev_attr_glove_mode,
	&dev_attr_keyboard_mode,
	&dev_attr_flip_mode,
#ifdef CONFIG_TOUCHKEY_LIGHT_EFS
	&dev_attr_touchkey_light_version,
	&dev_attr_touchkey_light_update,
	&dev_attr_touchkey_light_id_compare,
	&dev_attr_touchkey_light_table_write,
#endif
	NULL,
};
			...
			struct abov_touchkey_platform_data *pdata)
{
	...
	pdata->gpio_en = of_get_named_gpio_flags(np, "abov,gpio_en",0, &flags);
	if (pdata->gpio_en < 0) {
		printk("fail to get gpio_en\n");
		pdata->gpio_en = 0;
	} else {
		ret = gpio_request(pdata->gpio_en, "abov_gpio_en");
		if (ret < 0) {
				  ...
				  const struct i2c_device_id *id)
{
	...
	/*info->dev = sec_device_create(info, "sec_grip");
	if (IS_ERR(info->dev))
		input_err(true, &client->dev,
	...
	*/
	ret = sensors_register(&info->dev,
		info, sec_touchkey_attributes, MODULE_NAME);
	if (ret) {
		pr_err("[abov_a96t316] %s - cound not register sensor(%d).\n",
			__func__, ret);
	}
