...
#elif defined(WLAN_OPEN_SOURCE) && defined(CONFIG_WAKELOCK)
				 ...
				 uint32_t reason)
{
	if (pLock->is_initialized) {
		...
		/*
		 * Dont prevent Autosuspend for these reasons, either it is
		 * not required to do so or runtime functionality is not
					 ...
					 uint32_t reason)
{
	...
	*/
	if (pLock->is_initialized) {
		...
		vos_runtime_pm_prevent_suspend_timeout(
					pLock->runtime_pm_context, msec);
#if defined CONFIG_CNSS
...
#endif
		/*
		 * Dont allow autosuspend for these reasons, these reasons
		 * doesn't prevent the autosuspend so no need to call allow.
		 */
		switch(reason) {
		case WIFI_POWER_EVENT_WAKELOCK_DRIVER_INIT:
		case WIFI_POWER_EVENT_WAKELOCK_DRIVER_REINIT:
