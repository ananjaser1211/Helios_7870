...
//		sms_debug("wakeup.\n");
	...
	sms_err("smsspi thread exiting with code %d\n", ret);
...
static int smsspi_postload(void *context)
{
	...
	if ( (mode != SMSHOSTLIB_DEVMD_ISDBT) &&
		...
		fwDnlComplete(spi_device->phy_dev, 0);		/* jgk: is it required for mode(-1)? */
	}
...
static int smsspi_identify_device(struct _spi_device_st *spi_device)
{
	...
	sms_err("SPI Common initialized.\n");
