		...
		int unused_bytes)
{
	...
	tmp_buf = dev->cb.allocate_rx_buf(dev->context,		/* allocate_rx_buf() */
		RX_PACKET_SIZE);
	if (!tmp_buf) {
		PRN_ERR((TXT
				...
				int padding_allowed)
{
	...
	len = 0;
	if (!dev->cb.transfer_data_cb) {		/* smsspibus_xfer() */
		PRN_ERR((TXT
	...
	unused_bytes = 0;
	buf =
	    dev->cb.allocate_rx_buf(dev->context,					/* allocate_rx_buf() */
	...
	while (bytes_to_transfer || unused_bytes) {
		if ((unused_bytes <= 0) && (bytes_to_transfer > 0)) {
			...
			dev->cb.transfer_data_cb(dev->phy_context,					/* smsspibus_xfer() */
					(unsigned char *)txbuf,
					tx_phy_addr,
	...
	dev->cb.free_rx_buf(dev->context, buf);				/* free_rx_buf() */
}
