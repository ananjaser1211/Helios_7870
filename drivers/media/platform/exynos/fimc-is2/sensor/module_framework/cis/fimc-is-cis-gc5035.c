...
int sensor_gc5035_cis_init(struct v4l2_subdev *subdev)
{
    ...
    /*In Joshua "sensor_gc5035_cis_set_global_setting member" is not part of "fimc_is_cis_ops cis_ops" , so make changes according to joshua*/
	/* setfile global setting is at camera entrance */
	ret = sensor_cis_set_registers(subdev, sensor_gc5035_global, sensor_gc5035_global_size);
	if (ret < 0) {
...
int sensor_gc5035_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	ret = sensor_cis_set_registers(subdev, sensor_gc5035_setfiles[mode], sensor_gc5035_setfile_sizes[mode]);
	if (ret < 0) {
		err("sensor_gc5035_set_registers fail!!");
		goto p_err;
	}
