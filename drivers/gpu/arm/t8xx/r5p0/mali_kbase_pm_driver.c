...
/* MALI_SEC_INTEGRATION */
/* This powered flag set is a bug because if the flag set after init_hw,
 * the init_hw function calls power_on function again.
 */
...
/* MALI_SEC_INTEGRATION */
	if(kbdev->vendor_callbacks->hwcnt_power_up)
		kbdev->vendor_callbacks->hwcnt_power_up(kbdev);
}
...
void kbase_pm_clock_off(struct kbase_device *kbdev, mali_bool is_suspend)
{
	...
	/* MALI_SEC_INTEGRATION */
	if(kbdev->vendor_callbacks->hwcnt_power_down)
		kbdev->vendor_callbacks->hwcnt_power_down(kbdev);
...
mali_error kbase_pm_init_hw(struct kbase_device *kbdev, mali_bool enable_irqs)
{
	...
	if (kbdev->pm.callback_power_change_dvfs_level)
		kbdev->pm.callback_power_change_dvfs_level(kbdev);
