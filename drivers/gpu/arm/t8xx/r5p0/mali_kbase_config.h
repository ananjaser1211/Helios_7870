...
enum {
	...
	/**
	 * Attached value: pointer to @ref vendor_callbacks
	 */
...
/* MALI_SEC_INTEGRATION */
/*
 * @brief Specifies platform specific functions for chip-vendor callbacks.
 * This structure is defined at gpu_integration_defs.h
 */
typedef struct kbase_vendor_callbacks {
	void (* create_context)(void *kctx);
	void (* destroy_context)(void *kctx);
	void (* pm_metrics_init)(void *kbdev);
	void (* pm_metrics_term)(void *kbdev);
	void (* cl_boost_init)(void *kbdev);
	void (* cl_boost_update_utilization)(void *kbdev, void *katom, u64 microseconds_spent);
	void (* fence_timer_init)(void *atom);
	void (* fence_del_timer)(void *atom);
	int (* get_core_mask)(void *kbdev);
	int (* init_hw)(void *kbdev);
	void (* hwcnt_init)(void *kbdev);
	void (* hwcnt_remove)(void *kbdev);
	void (* hwcnt_prepare_suspend)(void *kbdev);
	void (* hwcnt_prepare_resume)(void *kbdev);
	mali_error (* hwcnt_update)(void *kbdev);
	void (* hwcnt_power_up)(void *kbdev);
	void (* hwcnt_power_down)(void *kbdev);
	void (* set_poweron_dbg)(mali_bool enable_dbg);
	mali_bool (* get_poweron_dbg)(void);
	void (* debug_pagetable_info)(void *kctx, mali_addr64 vaddr);
	void (* jd_done_worker)(void *kbdev);
	void (* update_status)(void *kbdev, char *str, u32 val);
	void (* mem_init)(void *kbdev);
	mali_bool (* mem_profile_check_kctx)(void *kctx);
	void (* pm_record_state)(void *kbdev, mali_bool is_active);
} kbase_vendor_callbacks;
