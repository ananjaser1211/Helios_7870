...
uintptr_t kbasep_get_config_value(struct kbase_device *kbdev, const struct kbase_attribute *attributes, int attribute_id)
{
	...
	case KBASE_CONFIG_ATTR_VENDOR_CALLBACKS:
		return 0;
...
/* MALI_SEC_INTEGRATION */
static mali_bool kbasep_validate_vendor_callback(const struct kbase_vendor_callbacks *callbacks, const struct kbase_device *kbdev)
{
	if (callbacks == NULL) {
		/* Having no callbacks is valid */
		return MALI_TRUE;
	...
	if(callbacks->create_context == NULL)
	{
		dev_warn(kbdev->dev, "There is no create context callbacks");
		return MALI_FALSE;
	}

	if(callbacks->destroy_context == NULL)
	{
		dev_warn(kbdev->dev, "There is no create context callbacks");
		return MALI_FALSE;
	}
...
mali_bool kbasep_validate_configuration_attributes(struct kbase_device *kbdev, const struct kbase_attribute *attributes)
{
	...
	case KBASE_CONFIG_ATTR_PM_POWEROFF_TICK_GPU:
		...
		/* MALI_SEC_INTEGRATION */
		case KBASE_CONFIG_ATTR_VENDOR_CALLBACKS:
			if (MALI_FALSE == kbasep_validate_vendor_callback((struct kbase_vendor_callbacks *)attributes[i].data, kbdev)) {
				/* Warning message handled by kbasep_validate_pm_callback() */
				return MALI_FALSE;
			}
			break;
