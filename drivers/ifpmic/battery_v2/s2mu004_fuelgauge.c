...
static void s2mu004_fg_test_read(struct i2c_client *client)
{
	...
	/* address 0x21 */
	s2mu004_read_reg_byte(client, 0x21, &data);
	sprintf(str+strlen(str),"0x21:0x%02x, ",data);
	...
	/* Temporarily print 0x92 ~ 0xFF to check SOC decrease issue*/
	/* Delete below when applying */
	for (i = 0x92; i <= 0xAF; i++) {
		s2mu004_read_reg_byte(client, i, &data);
		sprintf(str + strlen(str), "0x%02x:0x%02x, ", i, data);
	}
